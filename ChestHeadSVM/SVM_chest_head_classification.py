...
def toSpectorgramAndPSD(signal, freq, low, high): #generate spectrogram and PSD
    ...
    spec = (list(map(lambda a : np.abs(a)**2,fft(signal))))
...
def features(signal, freq, high, low): #feature Extractions
    ...
    def SPD():
        ...
        return (sum(PSD[cut2000:cut6000])/sum(PSD[cut100:cut2000]), sum(PSD[cut1000:cut6000])/sum(PSD[cut100:cut1000]))
    def kurtosis():
    ...
    return (harmonicity(), r1, r2, kurtosis(), SC, SS)
...
def toCSV(soundTrack, freqMax, length):
    ...
    for sound in soundTrack:
        ...
        for i in range(0,len(track), 2048):
            curr = track[i:i+2048]
            H, r1, r2, K, SC, SS = features(curr, 44100, 10000, 60)
            data["H"].append(H)
            data["r1"].append(r1)
            data["r2"].append(r2)
            data["K"].append(K)
...
def cut(soundTrack, length): #cut sound track into several segment, each with their coreesponding chestHead value (0 = chest, 1 = head, 2 = noise)
...
#track = cut(soundTrack.T[0], 18.860408)
#data = toCSV(track, rate, 18.860408)# the full length of the sound track
#lot(data)

soundTrack = soundTrack.T[0]
soundTrack = soundTrack[int(0.265*len(soundTrack)):int(0.265*len(soundTrack))+2048]
for sample in  soundTrack:
    print(sample, end=",")
#print("++++++++++++++++++++++++=")
#print(features(soundTrack, 44100, 10000, 100))
